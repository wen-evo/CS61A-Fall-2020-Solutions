...
def balanced(m):
    ...
    if is_planet(m):
        ...
        if (is_planet(left_end) and is_planet(right_end)):
            ...
            #RECURSION
            arms_balanced = [balanced(left_end), balanced(right_end), total_weight(left_end) * length(left(m)) == total_weight(right_end) * length(right(m))]
            return all(arms_balanced)
...
def quadratic(x, a, b, c):
    ...
    def extreme_point():
        """Returns the t value where f(t) is minimum."""
        return -b/(2*a)
        
    minimum_x = extreme_point()
    p1 = f(lower_bound(x))
    p2 = f(upper_bound(x))
    
    if lower_bound(x) <= minimum_x <= upper_bound(x):
        # minimum in between
        min_out = f(minimum_x)
        
        return interval(min([min_out, p1, p2]),
                        max([min_out, p1, p2]))
    else:
        # minimum not in between interval
        p1 = f(lower_bound(x))
        p2 = f(upper_bound(x))
        return interval(min([p1, p2]),
                        max([p1, p2]))
