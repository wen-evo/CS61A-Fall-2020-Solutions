class VendingMachine:
    ...
    def restock(self, amount):
        self.amount += amount
        return f"Current {self.item} stock: {self.amount}"
...
def store_digits(n):
    ...
    "*** YOUR CODE HERE ***"
    if n >= 10:
        remaining, last = n // 10, n % 10
        other = store_digits(remaining)
        other_copy = other
        while other_copy.rest is not Link.empty:
            other_copy = other_copy.rest
        other_copy.rest = Link(last)
        return other
    else:
        return Link(n)
...
def is_bst(t):
    ...
    if len(t.branches) > 2:
        return False
    if len(t.branches) == 2:
        if not bst_min(t.branches[0]) <= t.label <= bst_max(t.branches[1]):
            return False
    for b in t.branches:
        if not is_bst(b):
            return False
