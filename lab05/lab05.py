...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_list = [distance(target_city, city) for city in [city_a, city_b]]
    min_idx = dis_list.index(min(dis_list))
    return get_name([city_a, city_b][min_idx])
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        if is_leaf(t1):
            ...
            if len_t1 == len_t2:
                ...
                return tree(label(t1)+label(t2), new_branches + branches(t1)[len_t2:])
            else:
                return tree(label(t1)+label(t2), new_branches + branches(t2)[len_t1:])
...
def build_successors_table(tokens):
    ...
    for word in tokens:
        ...
        "*** YOUR CODE HERE ***"        
        """
        table.setdefault(prev, []).append(word)
