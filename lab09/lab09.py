...
def subseqs(s):
    ...
    if not s:
        ...
        part = subseqs(s[1:])
        rest.extend(part)
        rest.extend(insert_into_all(s[0], part))
        return rest
    """
    # Solution #2
    if not s:
...
def insert(link, value, index):
    ...
    """
    if index == 0:
        old_value = link.first
        old_rest = link.rest
        link.first = value
        link.rest = Link(old_value, old_rest)
...
def prune_small(t, n):
    ...
    if t.is_leaf():
        ...
        if len(t.branches) > n:
            branches_to_keep = sorted(t.branches, key=lambda b: b.label)[:n]
            t.branches = branches_to_keep
        for b in t.branches:
            prune_small(b, n)
    """
